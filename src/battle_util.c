...
u8 TrySetCantSelectMoveBattleScript(void)
{
    ...
    if (((GetBattlerAbility(gActiveBattler) == ABILITY_GORILLA_TACTICS) || GetBattlerAbility(gActiveBattler) == ABILITY_OBSESSION || GetBattlerAbility(gActiveBattler) == ABILITY_BULL_RUSH)
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        //Bull Rush
            ...
            unusableMoves |= gBitTable[i];
        else if (GetBattlerAbility(battlerId) == ABILITY_OBSESSION && *choicedMove != MOVE_NONE && *choicedMove != 0xFFFF && *choicedMove != gBattleMons[battlerId].moves[i])
            unusableMoves |= gBitTable[i];
    }
...
{
    [ENUM_WEATHER_RAIN] = {B_WEATHER_RAIN_TEMPORARY, B_WEATHER_RAIN_PERMANENT, HOLD_EFFECT_THERMOSTAT},
    [ENUM_WEATHER_RAIN_PRIMAL] = {B_WEATHER_RAIN_PRIMAL, B_WEATHER_RAIN_PRIMAL, HOLD_EFFECT_THERMOSTAT},
    [ENUM_WEATHER_SUN] = {B_WEATHER_SUN_TEMPORARY, B_WEATHER_SUN_PERMANENT, HOLD_EFFECT_THERMOSTAT},
    [ENUM_WEATHER_SUN_PRIMAL] = {B_WEATHER_SUN_PRIMAL, B_WEATHER_SUN_PRIMAL, HOLD_EFFECT_THERMOSTAT},
    [ENUM_WEATHER_SANDSTORM] = {B_WEATHER_SANDSTORM_TEMPORARY, B_WEATHER_SANDSTORM_PERMANENT, HOLD_EFFECT_THERMOSTAT},
    [ENUM_WEATHER_HAIL] = {B_WEATHER_HAIL_TEMPORARY, B_WEATHER_HAIL_PERMANENT, HOLD_EFFECT_THERMOSTAT},
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_RAIN_DISH:
                ...
                {
                    BattleScriptPushCursorAndCallback(BattleScript_RainDishActivates);
                    gBattleMoveDamage = gBattleMons[battler].maxHP / 5; //rain dish heals 1/5th;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    effect++;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_STENCH:
         ...
         case ABILITY_VAMPIRE:
            ...
            {
                gBattleMoveDamage = GetDrainedBigRootHp(gBattlerAttacker, gBattleMoveDamage) / 3;
                if (gBattleMons[gBattlerAttacker].hp < gBattleMons[gBattlerAttacker].maxHP)
                {
                    gLastUsedAbility = ABILITY_VAMPIRE;
                    PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_VampireHPDrain;
                    effect++;
                }
            }
            break;
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battlerId].hp)
        {
            ...
            LEFTOVERS:
                ...
                break;
            case HOLD_EFFECT_FEAST:
                if (gBattleMons[battlerId].hp < gBattleMons[battlerId].maxHP && !moveTurn)
                {
                    gBattleMoveDamage = gBattleMons[battlerId].maxHP / 6;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    BattleScriptExecute(BattleScript_ItemHealHP_End2);
...
bool32 IsBattlerProtected(u8 battlerId, u16 move)
{
    ...
    if (GetBattlerAbility(gBattlerAttacker) == ABILITY_INFILTRATOR)
        return FALSE;
    else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_UNSEEN_FIST
            && (gBattleMoves[move].flags & FLAG_MAKES_CONTACT
            || (gBattleMoves[move].effect == EFFECT_SHELL_SIDE_ARM
            && gSwapDamageCategory)))
        return FALSE;
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    else if (gBattleMoves[move].effect == EFFECT_MIND_FLAY)
    {
    ...
    case ABILITY_LIMBER:
        if (moveType == TYPE_FLYING)
        {
            MulModifier(&modifier, UQ_4_12(0.5));
            if (updateFlags)
                RecordAbilityBattle(battlerDef, ABILITY_LIMBER);
        }
        break;
    case ABILITY_IMMUNITY:
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
                //removed this effect but kept if logic in case adding new abilities later
            break;
        }
    }
    ...
    case HOLD_EFFECT_CHOICE_SPECS:
        if (IS_MOVE_SPECIAL(move))
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
    case HOLD_EFFECT_WEIGHTED_BOOTS:
        if (gBattleMons[battlerAtk].hp <= gBattleMons[battlerAtk].maxHP / 3)
            MulModifier(&modifier, UQ_4_12(2.0));
        else
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    if ((gBattleMoves[move].effect == EFFECT_PSYSHOCK 
        || IS_MOVE_PHYSICAL(move)) // uses defense stat instead of sp.def
        && gBattleMoves[move].effect != EFFECT_SOUL_SHOCK)
    {
        defStat = def;
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, u16 typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    case ABILITY_FRIEND_GUARD:
        if (IsDoubleBattle() && IsBattlerAlive(BATTLE_PARTNER(abilityDef)))
    ...
    case HOLD_EFFECT_THROAT_SPRAY:
        ...
        break;
    case HOLD_EFFECT_BRASS_KNUCKLES:
        if (gBattleMoves[move].flags & FLAG_MAKES_CONTACT)
        {
            MulModifier(&finalModifier, UQ_4_12(1.25));
        }
...
static u16 CalcTypeEffectivenessMultiplierInternal(u16 move, u8 moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier)
{
    ...
    else if (moveType == TYPE_FAIRY && recordAbilities && GetBattlerAbility(battlerDef) == ABILITY_SKEPTIC)
    {
        modifier = UQ_4_12(0.0);
        gLastUsedAbility = ABILITY_SKEPTIC;
        gMoveResultFlags |= (MOVE_RESULT_MISSED | MOVE_RESULT_DOESNT_AFFECT_FOE);
        gLastLandedMoves[battlerDef] = 0;
