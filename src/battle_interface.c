...
{   // Corresponds to gHealthboxElementsGfxTable (and the tables after it) in graphics.c
    ...
    HEALTHBOX_GFX_34,
    HEALTHBOX_GFX_35,
    HEALTHBOX_GFX_STATUS_TXC_BATTLER0,
    HEALTHBOX_GFX_119,
    HEALTHBOX_GFX_120,
    ...
    HEALTHBOX_GFX_84,
    HEALTHBOX_GFX_85,
    HEALTHBOX_GFX_STATUS_TXC_BATTLER1,
    HEALTHBOX_GFX_122,
    HEALTHBOX_GFX_123,
    ...
    HEALTHBOX_GFX_99,
    HEALTHBOX_GFX_100,
    HEALTHBOX_GFX_STATUS_TXC_BATTLER2,
    HEALTHBOX_GFX_124,
    HEALTHBOX_GFX_125,
    ...
    HEALTHBOX_GFX_114,
    HEALTHBOX_GFX_115,
    HEALTHBOX_GFX_STATUS_TXC_BATTLER3,
    HEALTHBOX_GFX_126,
    HEALTHBOX_GFX_127,
...
void SwapHpBarsWithHpText(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        if (GetBattlerSide(i) == B_SIDE_PLAYER && IsDoubleBattle())
        {
            ...
            if (noBars == TRUE && IsDoubleBattle()) // bars to text
            {
        ...
        else if (GetBattlerSide(i) == B_SIDE_OPPONENT)
        {
            ...
            else // text to bars
            {
                ...
                CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END_BAR), (void*)(OBJ_VRAM0 + 0x660 + gSprites[gHealthboxSpriteIds[i]].oam.tileNum * TILE_SIZE_4BPP), 32);
            }
        }
    }
}
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    {
        statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_TXC_BATTLER0, battlerId));
        statusPalId = PAL_STATUS_TXC;
    }
    else
    {
        ...
        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars || (GetBattlerSide(battlerId) == B_SIDE_PLAYER && !IsDoubleBattle()))
    ...
    volatile u8 testId = battlerId;
    volatile u32 pltAddertest = pltAdder;
...
static u8 GetStatusIconForBattlerId(u8 statusElementId, u8 battlerId)
{
    ...
    case HEALTHBOX_GFX_STATUS_BRN_BATTLER0:
        ...
        break;
    case HEALTHBOX_GFX_STATUS_TXC_BATTLER0:
        if (battlerId == 0)
            ret = HEALTHBOX_GFX_STATUS_TXC_BATTLER0;
        else if (battlerId == 1)
            ret = HEALTHBOX_GFX_STATUS_TXC_BATTLER1;
        else if (battlerId == 2)
            ret = HEALTHBOX_GFX_STATUS_TXC_BATTLER2;
        else
            ret = HEALTHBOX_GFX_STATUS_TXC_BATTLER3;
        break;
    }
    return ret;
}
...
void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon, u8 elementId)
{
    ...
    else
    {
        ...
        if (gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars && (elementId == HEALTHBOX_MAX_HP || elementId == HEALTHBOX_ALL))
            UpdateHpTextInHealthbox(healthboxSpriteId, GetMonData(mon, MON_DATA_MAX_HP), HP_MAX);
        if ((elementId == HEALTHBOX_HEALTH_BAR || elementId == HEALTHBOX_ALL) && !gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        {
            LoadBattleBarGfx(0);
            maxHp = GetMonData(mon, MON_DATA_MAX_HP);
            currHp = GetMonData(mon, MON_DATA_HP);
...
s32 MoveBattleBar(u8 battlerId, u8 healthboxSpriteId, u8 whichBar, u8 unused)
{
    ...
    if (whichBar == EXP_BAR || (whichBar == HEALTH_BAR && !gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        || (whichBar == HEALTH_BAR && GetBattlerSide(battlerId) == B_SIDE_PLAYER && !IsDoubleBattle()))
