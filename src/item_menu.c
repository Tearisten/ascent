...
static void BagMenu_ItemPrintCallback(u8, s32, u8, u8);
...
static void BagMenu_ItemPrintCallback(u8 windowId, s32 itemIndex, u8 y, u8 itempos)
{
    u16 itemId;
    u16 itemQuantity;
    int offset;
    ...
    if (itemIndex != LIST_CANCEL)
    {
        ...
        else if (gBagPosition.pocket != KEYITEMS_POCKET && ItemId_GetImportance(itemId) == FALSE && gBagPosition.pocket != TMHM_POCKET)
        {
            // Print item quantity
...
static void Task_ItemContext_SingleRow(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        if (gBagPosition.pocket + 1 == POCKET_BERRIES)
        {
            sItemMenuActions[ACTION_GIVE].func.void_u8(taskId);
        }
...
static void Task_ItemContext_MultipleRows(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        PlaySE(SE_SELECT);
        // for use items
        if (gBagPosition.pocket + 1 == POCKET_POKE_BALLS)
        {
            sItemMenuActions[ACTION_USE].func.void_u8(taskId);
        }
        // for hold items
        else if (gBagPosition.pocket + 1 == POCKET_ITEMS)
        {
            sItemMenuActions[ACTION_GIVE].func.void_u8(taskId);
        }
    }
...
static const u16 sItemsByType[ITEMS_COUNT] =
{
    ...
    [ITEM_RETRO_MAIL] = ITEM_TYPE_MAIL,
        ...
        [ITEM_MAGIC_HAT] = ITEM_TYPE_HELD_ITEM,
